...
import { Collection, Product } from '@avc/generated/graphql'
...
export default function UpdateCollectionForm({
  ...
  const assets = collection.featuredAsset
    ? [{ ...collection.featuredAsset, featured: true }, ...collection.documents]
  ...
  const updateCollectionMutation = async (data: any) => {
    ...
    try {
      ...
      if (updatedCollection) {
        ...
        router.push(`/katalog/koleksiyonlar/${updatedCollection.id}`)
      }
    } catch (error) {
      console.error(error)
    }
